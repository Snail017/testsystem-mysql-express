<template>
  <ul class="navtop nav">
    ...
    <li @click.stop="add(2)">
      <i class="icon iconfont icon-danxuankuang-copy"></i>多选
    </li>
    <li @click.stop="add(3)">
      <i class="icon iconfont icon-plus-judgement"></i>判断
    </li>
    <li @click.stop="add(0)">
      <i class="icon iconfont icon-plus-shortanswer"></i>简答
    </li>
  </ul>
</template>
...
export default {
  ...
  methods: {
    ...
    add(type) {
      ...
      if (type == 3 && ls_data.optiondata.length == 1) {
        //当为判断/3时  选项格式改变 固定为两个选项
...
.navtop li:nth-last-of-type(1):after {
  display: none;
}
.navtop li:hover {
  background: orange;
