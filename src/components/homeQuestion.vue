<template>
    <div class="form-horizontal answer_body">
        <div class="form-group row answer_head">
            <div class="col-sm-2">
                <router-link tag="span"  class="btn btn-outline-primary"   to="/questionnaire"><i class="iconfont icon-76xinzeng"></i>我的问卷</router-link>
            </div>
        ...
        <template v-if="testdata!=''">
            <div  class="form-group row answer_list" v-for="(item,index) in testdata"  :class="{'bg_yellow':item.status==0,'bg_green':item.status==1}">
                <div class="col-sm-5 row"><span class="col-sm-3">ID:{{item.id}}</span><span class="col-sm-8 st_title">{{item.title}}</span>  </div>
                <div class="col-sm-7" style="text-align: right">
                    <span class="st_btn green" v-if="item.status==0" @click.stop="ExamStatus(item.id,1)"><i class="iconfont icon-weibiaoti--1"></i>发布考试</span>
                    <span class="st_btn lightgreen" v-if="item.status==0" @click.stop="ExamStatus(item.id,2)"><i class="iconfont icon-weibiaoti--1"></i>作废</span>
                    <router-link tag="span" :to="{path:'/questionnaire',query:{id:item.id}}"  class="st_btn lightgreen" v-if="item.status!=2" @click.stop="ExamStatus(item.id,2)"><i class="iconfont icon-weibiaoti--1"></i>编辑</router-link>
                    <router-link tag="span" :to="{path:'/answer_list',query:{exam_id:item.id}}" class="st_btn green" v-if="item.status!=0||examType==3" ><i class="iconfont icon-yanjing"></i>答题情况</router-link>
                    <span class="st_btn blue" v-if="item.status==1" @click.stop="ExamStatus(item.id,0)"><i class="iconfont icon-weibiaoti--"></i>结束考试</span>
                    <span class="st_btn blue" @click.stop="deleteExam(item.id,index)"  v-if="item.status!='1'"><i class="iconfont icon-huishouzhan"></i>删除</span>
                    <span v-if="item.status=='1'"><i class="iconfont icon-wancheng1"></i>完成{{item.answer_count}}人</span>&nbsp;
                ...
                handler(newVal) {
                    ...
                    switch (newVal) {
                        ...
                        case 'unfinish':
                            ...
                            break;
                        case 'search':
                            this.examTypeUrl='/Exam/GetList?status=&title='+_this.title+'';
                            break;
                    }
                    this.Exam(1);
                },
...
<style scoped>
    ...
    @media screen and (max-width: 800px) {
        .answer_body{width: 100%;padding: 10px;margin: 0;}
    }
</style>